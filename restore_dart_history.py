...
now_secs = time.time()
twenty_four_hours_ago_secs = now_secs - (24 * 3600)
                        ...
                        if "caculateapp" in file_path.lower():
                            ...
                            if latest:
                                ...
                                if not os.path.exists(file_path):
                                    if latest_ts >= twenty_four_hours_ago_secs:
                                        count += 1
                                        latest_id = latest[-1].get("id")
                                        src_file = os.path.join(folder_path, latest_id)
                                        print(f"To restore: {file_path} (Deleted at: {time.ctime(latest_ts)})")
...
print(f"Total dart files restored from VSCode/Cursor History within 24h: {restored}")
