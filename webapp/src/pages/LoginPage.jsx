...
// อนุญาตเฉพาะ path ภายใน shell กัน open redirect
...
export default function LoginPage() {
  ...
  const [tab, setTab] = useState('line') // line | pin | phone
  const handledCode = useRef(false)

  // ล็อกอินอยู่แล้ว → เด้งไปหน้าที่ตั้งใจไว้
  useEffect(() => {
    if (user) navigate(redirectTo, { replace: true })
  }, [user, navigate, redirectTo])

  // LINE OAuth callback: /app/login?code=...&state=...
  useEffect(() => {
    ...
    api.auth.createSession({
      ...
      .finally(() => setBusy(''))
  }, [params, login, navigate, redirectTo])
